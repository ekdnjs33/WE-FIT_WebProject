    ...
    <div class="content-wrapper">
      ...
      <div class="container" align="center" style="margin-top: 50px;"> <!--style추가 다원-->
        ...
        <table class="table table-light table-bordered table-striped" style="border-color: #fff; width: 1000px; font-family: 'a고딕13'; ">
          ...
          <?php
          $sql = mysqli_query($db, "SELECT email, (@real_rank := IF(@last > old_score, @real_rank := @real_rank+1, @real_rank)) AS real_rank, (@last := old_score) AS last FROM basicone AS a, (SELECT @last := 0, @real_rank := 1 ) AS b, users WHERE a.id = users.id ORDER BY a.old_score DESC"); //(@ranking := @ranking+1) AS ranking
